$(document).ready(() => {
  ...
  function displayArticles(response) {
    ...
    response.results.forEach((result, index) => {
      ...
      recentArticlesEl.append(
        `<li class ="hover:text-amber-100 hover:cursor-pointer hover:text-lg border-b-2 border-amber-50" data-url="${url}">${
          index + 1
        }.${title}</li>`
      );
    });
  ...
  function displayVideo(response) {
    ...
    if (src === "") {
      src =
        "https://video.gamespot.com/2022/10/14/7f80585a-d61b-49f2-a0ff-d0301e64934d/Feature_GodofLoreLoki_10142022_1080h5000k.mp4";
    }
  ...
  function displayTheGameDetails(data) {
    ...
    platforms.forEach((platform) => {
      platformsEl.append(
        `<div class="badge badge-secondary"> ${platform} </div>`
      );
    });
  ...
  function displayGames(data) {
    ...
    if (data.results.length === 0) {
      ...
      for (result of data.results) {
        const platforms = result.platforms.map(
  ...
  function fetchGames(gameQueryString) {
    ...
    searchMsgEl.html(
      `Showing results for: <span class="data-caption">${gameQueryString}</span>`
    );
  }
    ...
    if (recentSearches.length > 0) {
      recentSearches.forEach((search) => {
        recentSearchEl.append(
          `<li class ="hover:text-amber-100 hover:cursor-pointer hover:text-lg border-b-2 border-amber-50 ">${i}.${search}</li>`
        );
        i++;
      });
    }
